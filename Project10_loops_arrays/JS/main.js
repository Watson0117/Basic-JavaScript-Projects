...
function call_loop() {
    ...
    while (X < 11) {//this defins where the loop will start and how it will end. it should loop as long as X is less than 11
        Digit += "<br>" + X;//this sets the value of Digit after run. it starts as zero basicly and then gets X added to it makeing the value one.
...
function STL() {
    var Sen = "Hello World example.";//this string in 20 characters long and is a simple string value. it is set to the var Sen.
...
function for_loop() {//this for loop sets the values of bolth Y and Content as they work with each other.
    for (Y = 0; Y < Instruments.length; Y++) {//this sets the value of Y as the first option in the array or position 0. it then increments the position in the array by 1 
        Content += Instruments[Y] + "<br>";//this makes content = Y which has a value of the 0 position in the array at the start of the loop.
    }
...
function cat_pics(){
    ...
    document.getElementById("Cat").innerHTML = "In this picture, the cat is " +//this writes the string and then adds the object and propertys of that object to creat a full string.
    Cat_Picture[3] + ".";
}
